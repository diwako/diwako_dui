...
// Волна эхолокации по углу
private _waveSpeed = 180; // градусов в секунду
private _now = time;
private _waveAngle = (_now * _waveSpeed) % 360;
private _waveWidth = 30; // ±градусы от фронта волны

{
    ...
    if (_distance > _circleRange) exitWith {};

    private _rDir = ((((getPosVisual _player) getDir (getPosVisual _unit)) - _playerDir) + 360) % 360;
    private _relDir = (_rDir - (_viewDir - _playerDir)) mod 360;

    // Эффект волны: юниты вне волны не отображаются
    private _angleDiff = abs (_relDir - _waveAngle);
    if (_angleDiff > 180) then { _angleDiff = 360 - _angleDiff; };
    if (_angleDiff > _waveWidth) exitWith {};

    private _alpha = linearConversion [_circleRange * 0.90, _circleRange, _distance, diwako_dui_compass_opacity, 0, true];
    ...
    if (_alpha <= 0) exitWith { ctrlDelete _ctrl };

    private _dir = -(_viewDir - (getDirVisual _unit)) mod 360;
    private _divisor = linearConversion [35, 50, _circleRange, 2.25, 2.75, false] / diwako_dui_hudScaling;
