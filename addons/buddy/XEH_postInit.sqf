...
if (isNil "ace_interact_menu_fnc_createAction") then {
    ...
    }, [], -5000, false, true, "", format ["cursorObject distance2d player <= %1 && {cursorObject in (units group player) && {(player getVariable [QQGVAR(buddy), objNull]) != cursorObject}}", _range]]] call CBA_fnc_addPlayerAction;
    ...
    },{params ["_target", "_player"]; !(ace_interaction_EnableTeamManagement && [_player, _target] call ace_interaction_fnc_canJoinTeam) && {_target in (units group _player) && {(_player getVariable [QGVAR(buddy), objNull]) != _target}}},{},[], [0,0,0], _range] call ace_interact_menu_fnc_createAction;
    ...
    },{params ["_target", "_player"]; !ace_interaction_EnableTeamManagement && {!isNull (_player getVariable [QGVAR(buddy), objNull])}},{},[], [0,0,0]] call ace_interact_menu_fnc_createAction;
    ...
    _action = [QGVAR(buddy_action_team), _labelAdd, "", {
        params ["_target", "_player", "_params"];
        [_player, _target] call FUNC(pairBuddies);
    },{params ["_target", "_player"]; (ace_interaction_EnableTeamManagement && [_player, _target] call ace_interaction_fnc_canJoinTeam) && {(_player getVariable [QGVAR(buddy), objNull]) != _target}},{},[], [0,0,0], _range] call ace_interact_menu_fnc_createAction;
